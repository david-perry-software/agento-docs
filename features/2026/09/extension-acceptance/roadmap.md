...
- [ ] 3.1 Add `docs/extension.md` covering plugin-plus-VSIX installation, every view, refresh behavior, command routing, companion workspaces, recovery, and explicit limitations; align the extension developer README — verify: documented commands and setting names match `extension/package.json` and `extension/README.md` links resolve
- [ ] 3.2 Link the extension guide from the root README and expand the architecture documentation with the packaged-extension, generated-fixture, and dispatch-boundary acceptance model — verify: README and architecture links resolve and describe only behavior exercised by the acceptance suite
